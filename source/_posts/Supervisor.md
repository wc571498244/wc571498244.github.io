...
## 三、生成supervisor配置文件
**supervisor配置文件**
运行命令`echo_supervisord_conf`生成supervisor的初始配置文件:
```shell
echo_supervisord_conf > /etc/supervisord.conf # 生成在哪个地方可以自己选择
```
*注：supervisor的配置文件默认是不全的，不过在大部分默认的情况下，上面说的基本功能已经满足。*

**supervisor子进程配置文件**
需要修改配置文件`files`字段的路径(子进程配置文件路径)
```shell
...
;[include]
files = /etc/supervisord.d/*.ini # 子进程配置文件路径, 需要手动创建
```
